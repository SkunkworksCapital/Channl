...
<body>
  ...
  <div class="card">
    <h3>Recent Email Sent</h3>
    <?php
      $history = [];
      try {
        $h = db()->prepare('SELECT to_addr, status, provider_message_id, created_at FROM messages WHERE user_id = ? AND channel = "email" AND (status = "sent" OR status = "error") ORDER BY id DESC LIMIT 200');
