...
<body>
  ...
  <div class="card">
    ...
    <?php
      ...
      // Load lists and their default template bodies
      ...
      $stmt = db()->prepare('SELECT l.id, l.name, mt.name AS tpl_name, mt.body AS tpl_body FROM contact_lists l LEFT JOIN message_templates mt ON mt.id = l.default_sms_template_id AND mt.user_id = ? AND mt.type = "sms" WHERE l.user_id = ? AND l.channel = "sms" ORDER BY l.name');
      $stmt->execute([current_user_id(), current_user_id()]);
      $listRows = $stmt->fetchAll();
      $listTplMap = [];
      foreach ($listRows as $l) {
        if (!empty($l['tpl_body'])) { $listTplMap[(int)$l['id']] = ['name' => $l['tpl_name'], 'body' => $l['tpl_body']]; }
      }
      // Load all SMS templates for selection
      $tplStmt = db()->prepare('SELECT id, name, body FROM message_templates WHERE user_id = ? AND type = "sms" ORDER BY name');
      $tplStmt->execute([current_user_id()]);
      $tplRows = $tplStmt->fetchAll();
      $tplMap = [];
      foreach ($tplRows as $t) { $tplMap[(int)$t['id']] = ['name' => $t['name'], 'body' => $t['body']]; }
    ?>
  ...
  <script>
    (function(){
      ...
      function sync(){
        ...
        if (!tpl && list && list.value) {
          tpl = listTplMap[list.value] || listTplMap[parseInt(list.value, 10)];
        }
        if (tpl && bodyBulk) { bodyBulk.value = tpl.body; }
      ...
      async function fetchTemplate(id){
        try{
          ...
          if(data && data.ok && data.template && bodyBulk){ bodyBulk.value = data.template.body || ''; }
        }catch(e){}
      }
      ...
      function showRecipientsModal(members, total){
        ...
        var thead = document.createElement('thead'); var trh=document.createElement('tr'); ['Name','Phone','Email','Country'].forEach(function(t){ var th=document.createElement('th'); th.textContent=t; th.style.borderBottom='1px solid #374151'; th.style.textAlign='left'; th.style.padding='8px 6px'; trh.appendChild(th); }); thead.appendChild(trh); table.appendChild(thead);
        var tbody = document.createElement('tbody');
        members.forEach(function(m){ var tr=document.createElement('tr'); ['name','phone','email','country'].forEach(function(k){ var td=document.createElement('td'); td.textContent = (m && m[k]) ? m[k] : ''; td.style.borderBottom='1px solid #374151'; td.style.padding='8px 6px'; tr.appendChild(td); }); tbody.appendChild(tr); });
