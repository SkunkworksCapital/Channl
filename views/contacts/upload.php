...
<head>
  ...
  <title>Upload Contacts • channl</title>
  <style>
    ...
    .card{max-width:640px;margin:2rem auto;background:#111827;border:1px solid #374151;border-radius:12px;padding:24px}
    input[type="text"],input[type="file"]{width:100%;padding:10px;border-radius:8px;border:1px solid #374151;background:#0f172a;color:#e5e7eb}
...
<body>
  ...
  <div class="card">
    ...
    <form method="post" action="/contacts/upload" enctype="multipart/form-data">
      <input type="hidden" name="csrf" value="<?= h(csrf_token()) ?>">
      <label for="csv">CSV file (columns: name,email,phone,country)</label>
      <input id="csv" name="csv" type="file" accept=".csv" required>
      <label for="tags">Tags (comma separated, optional)</label>
      <input id="tags" name="tags" type="text" placeholder="customer, lead, 2025">
